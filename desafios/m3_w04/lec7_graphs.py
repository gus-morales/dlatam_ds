...
def plot_importance(fit_model, feat_names, n):
    ...
    plt.barh(range(len(feat_names[:n])), tmp_importance[sort_importance][:n])
    plt.yticks(range(len(feat_names[:n])), names, rotation=0)
